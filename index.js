...
input.addEventListener(`keyup`, e => {
  elementosFiltrados = events.filter(nombres => nombres.name.toLowerCase().includes(e.target.value.toLowerCase()))
  tarjetas.innerHTML = ''
...
checkbox.addEventListener(`change`, e =>{
    if (e.target.checked) {
        listaChequeada = listaChequeada.concat(events.filter(evento=> evento.category.toLowerCase().includes(e.target.id.toLowerCase())))
    ...
    listaChequeada = listaChequeada.filter(evento => !evento.category.toLowerCase().includes( e.target.id.toLowerCase() ) )
    tarjetas.innerHTML = ''
    imprimir(listaChequeada, tarjetas)
    }
